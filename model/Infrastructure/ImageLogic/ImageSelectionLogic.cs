...
namespace Infrastructure.ImageLogic
{
    public class ImageSelectionLogic
    {
        ...
        /// <returns></returns>
        private static bool HasAsset(Dictionary<string, string> keyValuePairs, string imageTypeRequired,
                            ...
                            foreach (var image in ApiAssetSortedList.Where(i => !i.expiredDateSpecified))
                            {
                                ...
                                if (!HasAsset(DbImagesForAsset, imageTypeRequired, image.URI))
                                {
                                    ...
                                    var match = Regex.Match(CurrentMappingData.GN_Images,
                                        $"(?m){imageTypeRequired}:.*?{imageExtension}");
                                    ...
                                    if (match.Success || match.Value == "")
                                    {
                                        ...
                                        var newUri = $"{imageTypeRequired}: {image.URI}";
                                        ...
                                        CurrentMappingData.GN_Images = match.Value == ""
                                            ? CurrentMappingData.GN_Images != string.Empty
                                                ...
                                                : CurrentMappingData.GN_Images = $"{imageTypeRequired}: {image.URI}"
                                            : CurrentMappingData.GN_Images =
                                                CurrentMappingData.GN_Images.Replace(match.Value, newUri);
